...
YCRCB_img = cv2.cvtColor(test_img, cv2.COLOR_BGR2YCR_CB)
...
new_img = cv2.calcBackProject([YCRCB_img], channels=[0,1], hist= model_hist, ranges=[0,180,0,256], scale=1)

new_img = convolve(new_img, r = 3)
_, new_img_thresh = cv2.threshold(new_img, 127, 255, cv2.THRESH_BINARY)
